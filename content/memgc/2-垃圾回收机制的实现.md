...
计算 $H_T$ 的最终结论（从 Go 1.10 时开始 $h_t$ 增加了上界 $0.95 \rho$，从 Go 1.14 开始时 $h_t$ 增加了下界 $0.6 \rho$）是：
...
- 特别的，当 $h_t^{(n)}<0.6 \rho$时，将其调整为 $0.6 \rho$，当 $h_t^{(n)} > 0.95 \rho$ 时，将其设置为 $0.95 \rho$（在默认 $\text{GOGC}=100$ 即 $\rho=1$ 时，下界即为 $0.6$）
