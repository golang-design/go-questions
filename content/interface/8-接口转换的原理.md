...
> **版本提示**：上面的汇编与源码基于较早的 Go 版本。随着编译器演进，这部分实现已经发生变化：
>
> - Go 1.18（CL 340029）重构了接口转换，`convI2I` 的签名变为只做 itab 查找：`func convI2I(dst *interfacetype, src *itab) *itab`，结果接口改由编译器在调用处构造。
> - Go 1.22（CL 531055）进一步在 itab 查找前加入了缓存，并**直接移除了 `convI2I`**。如今 `interface` 到（非空）`interface` 的转换会复用与类型断言相同的 `runtime.typeAssert` 机制：编译器在调用处内联一段 itab 缓存探测代码，只有在缓存未命中时才调用 `runtime.typeAssert`。
>
> 因此在新版本中反汇编 `r = c`，看到的不再是 `runtime.convI2I`，而是一段内联的缓存查找加上（慢路径上的）`runtime.typeAssert` 调用；如果编译器能证明该转换是平凡的（例如转换到自身或其超集接口），甚至可能没有任何运行时调用。但其核心逻辑——找到目标接口的 `tab` 并复用原有的 `data`——始终未变，下面的分析仍然适用。
